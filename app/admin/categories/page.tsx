...
import { FolderTree, Plus, Upload, Trash2, Edit, Loader2, X } from "lucide-react";
...
export default function CategoriesPage() {
  ...
  // 1. Fetch All Categories (Updated Endpoint)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/categories/add`, {
        method: "POST",
      ...
      if (response.ok || data.success) {
        ...
        setFormData({ name: "", description: "", isFeatured: "true", totalItems: "0" });
        removeSelectedImage();
        fetchCategories();
      } else {
  ...
  // 3. Delete Category (Updated Endpoint)
  const handleDelete = async (id: string) => {
    if (!window.confirm("Kya aap sach mein yeh category delete karna chahte hain?")) return;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pt-20 lg:pt-10 pl-4 lg:pl-72 pr-4 lg:pr-10 pb-10 transition-all duration-300">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl shadow-xl p-5 sm:p-6 h-fit">
          ...
          {errorMsg && <div className="mb-4 bg-red-50 text-red-600 border border-red-200 rounded-xl p-3 text-sm">{errorMsg}</div>}
          {successMsg && <div className="mb-4 bg-green-50 text-green-600 border border-green-200 rounded-xl p-3 text-sm">{successMsg}</div>}

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-semibold text-slate-800">Category Name</label>
              <input type="text" name="name" required value={formData.name} onChange={handleChange} placeholder="e.g., Kurtiya" className="w-full mt-1.5 p-3 text-slate-900 bg-white rounded-xl border border-slate-300 outline-none focus:ring-2 focus:ring-black text-sm" />
            </div>

            <div>
              <label className="text-sm font-semibold text-slate-800">Description</label>
              <textarea name="description" required rows={2} value={formData.description} onChange={handleChange} placeholder="Premium Collection..." className="w-full mt-1.5 p-3 text-slate-900 bg-white rounded-xl border border-slate-300 outline-none focus:ring-2 focus:ring-black resize-none text-sm" />
            </div>
            ...
            <div>
              <label className="text-sm font-semibold text-slate-800">Category Banner Image</label>
              {!imagePreview ? (
                ...
                <div className="mt-1.5 relative aspect-video w-full rounded-xl border overflow-hidden bg-slate-100 group">
                  ...
                  <button type="button" onClick={removeSelectedImage} className="absolute top-1.5 right-1.5 bg-red-500 text-white p-1 rounded-full shadow hover:bg-red-600 transition">
                    <X size={12} />
                  </button>
                </div>
              )}
            ...
            <button type="submit" disabled={submitting} className="w-full bg-black text-white py-3 rounded-xl font-semibold hover:bg-gray-800 transition text-sm flex items-center justify-center gap-2">
        ...
        {/* Right Panel: Clean Data List Grid Table */}
        <div className="bg-white border border-slate-200 rounded-3xl shadow-xl p-5 sm:p-6 lg:col-span-2">
          ...
          {loading ? (
            <div className="flex flex-col items-center py-12 gap-2">
              ...
              <p className="text-xs text-slate-500">Categories load ho rahi hain...</p>
            </div>
          ) : categories.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-12">Koi category available nahi hai.</p>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {categories.map((cat) => (
                    <tr key={cat._id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="p-3 font-medium text-slate-700 whitespace-nowrap">{cat.totalItems} Items</td>
                      <td className="p-3 whitespace-nowrap">
                        <span className={`inline-block text-[11px] px-2 py-0.5 rounded-full font-semibold ${
                          cat.isFeatured?.toString() === "true" ? "bg-green-50 text-green-600 border border-green-200" : "bg-slate-100 text-slate-500"
                        }`}>
                          {cat.isFeatured?.toString() === "true" ? "Yes" : "No"}
                        </span>
                      </td>
                      <td className="p-3">
