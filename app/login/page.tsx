  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.token) {
        ...
        // 2. SET IN COOKIES (Client-side Document Cookie)
        const days = 7;
        const expires = new Date(Date.now() + days * 864e5).toUTCString();
        document.cookie = `authToken=${data.token}; expires=${expires}; path=/; SameSite=Lax`;
  ...
  return (
    <div className="bg-[#fffaf8] min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans text-[#5c2430]">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] border border-[#efdcd2] shadow-sm p-8 sm:p-10 relative overflow-hidden">
        ...
        {/* Top Header: Brand Badge, Logo & Title */}
        <div className="text-center mb-8 space-y-2">
          ...
          <p className="text-[#8c6b63] text-xs uppercase tracking-widest font-semibold">
            Log in to your AVY Fashion account
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              <label className="text-[11px] font-bold uppercase tracking-wider text-[#5c2430]">
                Password
              </label>
              <Link
                href="/forgot-password"
                className="text-xs font-bold text-[#b5715f] hover:underline"
              >
                Forgot Password?
              </Link>
            </div>
